...
struct USVFSParameters SharedParameters::makeLocal() const {
    struct USVFSParameters result;
    USVFSInitParametersInt(&result, instanceName.c_str(), currentSHMName.c_str(), currentInverseSHMName.c_str(),
                           debugMode, logLevel);
    return result;
}

void usvfs::USVFSInitParametersInt(USVFSParameters* parameters, const char* instanceName, const char* currentSHMName,
...
HookContext::HookContext(const USVFSParameters& params, HMODULE module)
    ...
    spdlog::get("usvfs")->debug("context current shm: {0} (now {1} connections)", m_Parameters->currentSHMName.c_str(),
                                m_Parameters->userCount);

    s_Instance = this;

    if (m_Tree.get() == nullptr) {
        USVFS_THROW_EXCEPTION(usage_error() << ex_msg("shm not found") << ex_msg(params.instanceName));
    }
}
...
void HookContext::blacklistExecutable(const std::wstring& executableName) {
    m_Parameters->processBlacklist.insert(
        shared::StringT(shared::string_cast<std::string>(executableName, shared::CodePage::UTF8).c_str(),
...
void HookContext::registerDelayed(std::future<int> delayed) { m_Futures.push_back(std::move(delayed)); }

std::vector<std::future<int>>& HookContext::delayed() { return m_Futures; }

void HookContext::unlock(HookContext* instance) { instance->m_Mutex.signal(); }

void HookContext::unlockShared(const HookContext* instance) { instance->m_Mutex.signal(); }
