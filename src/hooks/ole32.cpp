            ...
            */
            ...
            if (processHandle != INVALID_HANDLE_VALUE) {
                ...
                if (GetProcessTimes(processHandle, &creationTime, &exitTime, &kernelTime, &userTime) == 0) {
                    ...
                    // the process was created before our call to CoCreateInstance so it can't have been created by
                    // that.
                    pid = 0;
                } else {
                    // process created after our call to CoCreateInstance. It's unlikely (though of course not
                    // impossible) that the process wasn't started by our call
                    try {
                        processName = winapi::ansi::getModuleFileName(nullptr, processHandle);
                    } catch (const std::exception& ex) {
                        spdlog::get("hooks")->warn("failed to retrieve module name ({1}): {0}", ex.what(),
                                                   processHandle);
                    }
...
#pragma message("update CoCreateInstanceEx once this works")
                        ...
                        injectProcess(context->dllPath(), context->callParameters(), processHandle,
...
HRESULT WINAPI usvfs::hooks::CoCreateInstanceEx(const IID& rclsid, IUnknown* punkOuter, DWORD dwClsCtx,
    ...
    if (dwClsCtx & CLSCTX_LOCAL_SERVER) {
        ...
        if (SUCCEEDED(procIdRes) && (pid != 0UL)) {
            ...
            if (processHandle != INVALID_HANDLE_VALUE) {
                HookContext::ConstPtr context = READ_CONTEXT();
                injectProcess(context->dllPath(), context->callParameters(), processHandle, INVALID_HANDLE_VALUE);
                try {
                    processName = winapi::ansi::getModuleFileName(nullptr, processHandle);
                } catch (const std::exception& ex) {
