...
#include "usvfs/hooks/sharedids.h"
#include "usvfs/loghelpers.h"
#include "usvfs/stringcast_boost.h"
...
std::ostream& operator<<(std::ostream& os, const _UNICODE_STRING& str) {
    try {
        ...
        // the string
        os << ush::string_cast<std::string>(str.Buffer, ush::CodePage::UTF8, str.Length / sizeof(WCHAR));
    } catch (const std::exception& e) {
        os << e.what();
    }
...
std::ostream& operator<<(std::ostream& os, POBJECT_ATTRIBUTES attr) { return operator<<(os, *attr->ObjectName); }

std::pair<UnicodeString, bool> applyReroute(const usvfs::HookContext::ConstPtr& context,
                                            const usvfs::HookCallContext& callContext, const UnicodeString& inPath) {
    ...
    if (callContext.active()) {
        ...
        std::string lookupPath =
            ush::string_cast<std::string>(static_cast<LPCWSTR>(result.first) + 4, ush::CodePage::UTF8);
        ...
        if ((node.get() != nullptr) && !node->data().linkTarget.empty()) {
            std::wstring reroutePath =
                ush::string_cast<std::wstring>(node->data().linkTarget.c_str(), ush::CodePage::UTF8);
...
std::pair<UnicodeString, UnicodeString> findCreateTarget(const usvfs::HookContext::ConstPtr& context,
                                                         const UnicodeString& inPath) {
    std::pair<UnicodeString, UnicodeString> result;
    result.first = inPath;
    result.second = UnicodeString();

    std::string lookupPath = ush::string_cast<std::string>(static_cast<LPCWSTR>(result.first) + 4, ush::CodePage::UTF8);
    FindCreateTarget visitor;
    usvfs::RedirectionTree::VisitorFunction visitorWrapper = [&](const usvfs::RedirectionTree::NodePtrT& node) {
        visitor(node);
    };
    context->redirectionTable()->visitPath(lookupPath, visitorWrapper);
    if (visitor.target.get() != nullptr) {
        fs::path relativePath = ush::make_relative(visitor.target->path(), fs::path(lookupPath));

        fs::path target(visitor.target->data().linkTarget.c_str());
        target /= relativePath;
...
void GetInfoData(LPCVOID address, FILE_INFORMATION_CLASS infoClass, ULONG& offset, std::wstring& fileName) {
    switch (infoClass) {
    case FileBothDirectoryInformation: {
        const FILE_BOTH_DIR_INFORMATION* info = reinterpret_cast<const FILE_BOTH_DIR_INFORMATION*>(address);
        offset = info->NextEntryOffset;
        fileName = std::wstring(info->FileName, info->FileNameLength / sizeof(WCHAR));
    } break;
    case FileDirectoryInformation: {
        const FILE_DIRECTORY_INFORMATION* info = reinterpret_cast<const FILE_DIRECTORY_INFORMATION*>(address);
        offset = info->NextEntryOffset;
        fileName = std::wstring(info->FileName, info->FileNameLength / sizeof(WCHAR));
    } break;
    case FileNamesInformation: {
        const FILE_NAMES_INFORMATION* info = reinterpret_cast<const FILE_NAMES_INFORMATION*>(address);
        offset = info->NextEntryOffset;
        fileName = std::wstring(info->FileName, info->FileNameLength / sizeof(WCHAR));
    } break;
    case FileIdFullDirectoryInformation: {
        const FILE_ID_FULL_DIR_INFORMATION* info = reinterpret_cast<const FILE_ID_FULL_DIR_INFORMATION*>(address);
        offset = info->NextEntryOffset;
        fileName = std::wstring(info->FileName, info->FileNameLength / sizeof(WCHAR));
    } break;
    case FileFullDirectoryInformation: {
        const FILE_FULL_DIR_INFORMATION* info = reinterpret_cast<const FILE_FULL_DIR_INFORMATION*>(address);
        offset = info->NextEntryOffset;
        fileName = std::wstring(info->FileName, info->FileNameLength / sizeof(WCHAR));
    } break;
    case FileIdBothDirectoryInformation: {
        const FILE_ID_BOTH_DIR_INFORMATION* info = reinterpret_cast<const FILE_ID_BOTH_DIR_INFORMATION*>(address);
        offset = info->NextEntryOffset;
        fileName = std::wstring(info->FileName, info->FileNameLength / sizeof(WCHAR));
    } break;
    case FileObjectIdInformation: {
    ...
    default: { offset = ULONG_MAX; } break;
    }
}
...
void SetInfoFilenameImplSN(T* info, const std::wstring& fileName) {
    ...
    if (info->ShortNameLength > 0) {
        info->ShortNameLength = static_cast<CCHAR>(GetShortPathNameW(fileName.c_str(), info->ShortName, 8));
    }
}

void SetInfoFilename(LPVOID address, FILE_INFORMATION_CLASS infoClass, const std::wstring& fileName) {
    switch (infoClass) {
    case FileBothDirectoryInformation: {
        SetInfoFilenameImplSN(reinterpret_cast<FILE_BOTH_DIR_INFORMATION*>(address), fileName);
    } break;
    case FileDirectoryInformation: {
        SetInfoFilenameImpl(reinterpret_cast<FILE_DIRECTORY_INFORMATION*>(address), fileName);
    } break;
    case FileNamesInformation: {
        SetInfoFilenameImpl(reinterpret_cast<FILE_NAMES_INFORMATION*>(address), fileName);
    } break;
    case FileIdFullDirectoryInformation: {
        SetInfoFilenameImpl(reinterpret_cast<FILE_ID_FULL_DIR_INFORMATION*>(address), fileName);
    } break;
    case FileFullDirectoryInformation: {
        SetInfoFilenameImpl(reinterpret_cast<FILE_FULL_DIR_INFORMATION*>(address), fileName);
    } break;
    case FileIdBothDirectoryInformation: {
        SetInfoFilenameImplSN(reinterpret_cast<FILE_ID_BOTH_DIR_INFORMATION*>(address), fileName);
    } break;
    default: {
        // NOP
    } break;
...
NTSTATUS addNtSearchData(HANDLE hdl, PUNICODE_STRING FileName, const std::wstring& fakeName,
                         FILE_INFORMATION_CLASS FileInformationClass, PVOID& buffer, ULONG& bufferSize,
                         std::set<std::wstring>& foundFiles, HANDLE event, PIO_APC_ROUTINE apcRoutine, PVOID apcContext,
    ...
    if (hdl != INVALID_HANDLE_VALUE) {
        ...
        res = NtQueryDirectoryFile(hdl, event, apcRoutine, apcContext, &status, buffer, bufferSize,
            ...
            while (totalOffset < status.Information) {
                ...
                if ((totalOffset == 0) && (offset == 0) && (fakeName.length() > 0)) {
                    ...
                    if ((fakeName.length() > fileName.length()) &&
                        ((fakeName.length() - fileName.length()) > (bufferSize - status.Information))) {
...
void gatherVirtualEntries(const UnicodeString& dirName, const usvfs::RedirectionTreeContainer& redir,
    ...
    // "\\?\"
    if ((wcsncmp(dirNameW, LR"(\\?\)", 4) == 0) || (wcsncmp(dirNameW, LR"(\??\)", 4) == 0)) {
        dirNameW += 4;
    }
    auto node = redir->findNode(fs::path(dirNameW));
    if (node.get() != nullptr) {
        std::string searchPattern =
            FileName != nullptr ? ush::string_cast<std::string>(FileName->Buffer, ush::CodePage::UTF8) : "*.*";
        ...
        for (const auto& subNode : node->find(searchPattern)) {
            if (((subNode->data().linkTarget.length() > 0) || subNode->isDirectory()) &&
                ...
                std::wstring vName = ush::string_cast<std::wstring>(subNode->name(), ush::CodePage::UTF8);

                Searches::Info::VirtualMatch m{
                    ush::string_cast<std::wstring>(subNode->data().linkTarget.c_str(), ush::CodePage::UTF8), vName};
 ...
 */
bool addVirtualSearchResult(PVOID& FileInformation, FILE_INFORMATION_CLASS FileInformationClass, Searches::Info& info,
                            const std::wstring& realPath, const std::wstring& virtualName, BOOLEAN ReturnSingleEntry,
                            ULONG& dataRead) {
    ...
    if (info.currentSearchHandle == INVALID_HANDLE_VALUE) {
        ...
        info.currentSearchHandle = CreateFileW(dirName.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
    ...
    NTSTATUS subRes = addNtSearchData(
        ...
        (fileName != L".") ? static_cast<PUNICODE_STRING>(UnicodeString(fileName.c_str())) : nullptr, virtualName,
        FileInformationClass, FileInformation, dataRead, info.foundFiles, nullptr, nullptr, nullptr, ReturnSingleEntry);
    if (subRes == STATUS_SUCCESS) {
        ...
        if (subRes != STATUS_NO_MORE_FILES) {
            spdlog::get("hooks")->warn("error reported listing files in {0}: {1:x}", fullPath.string(),
...
NTSTATUS WINAPI usvfs::hooks::NtQueryDirectoryFile(HANDLE FileHandle, HANDLE Event, PIO_APC_ROUTINE ApcRoutine,
    ...
    if (firstSearch) {
        ...
        infoIter = activeSearches.info.insert(std::make_pair(FileHandle, Searches::Info())).first;
        ...
        if (iter != searchMap.end()) {
            ...
            infoIter->second.currentSearchHandle =
                CreateFileW(iter->second.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr,
                            OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, nullptr);
        } else {
            searchPath = UnicodeString(FileHandle);
        }
        gatherVirtualEntries(searchPath, context->redirectionTable(), FileName, infoIter->second);
    }
    ...
    while (moreRegular && !dataReturned) {
        ...
        NTSTATUS subRes =
            addNtSearchData(handle, FileName, L"", FileInformationClass, FileInformationCurrent, dataRead,
    ...
    if (!moreRegular) {
        ...
        while (!dataReturned && infoIter->second.virtualMatches.size() > 0) {
            ...
            if (matchIter->realPath.size() != 0) {
                dataRead = Length;
                if (addVirtualSearchResult(FileInformationCurrent, FileInformationClass, infoIter->second,
                                           matchIter->realPath, matchIter->virtualName, ReturnSingleEntry, dataRead)) {
...
unique_ptr_deleter<OBJECT_ATTRIBUTES> makeObjectAttributes(std::pair<UnicodeString, bool>& redirInfo,
                                                           POBJECT_ATTRIBUTES attributeTemplate) {
    if (redirInfo.second) {
        unique_ptr_deleter<OBJECT_ATTRIBUTES> result(new OBJECT_ATTRIBUTES, [](OBJECT_ATTRIBUTES* ptr) { delete ptr; });
        ...
        return unique_ptr_deleter<OBJECT_ATTRIBUTES>(attributeTemplate, [](OBJECT_ATTRIBUTES*) {});
    }
}

NTSTATUS WINAPI usvfs::hooks::NtOpenFile(PHANDLE FileHandle, ACCESS_MASK DesiredAccess,
    ...
    if (((OpenOptions & FILE_DIRECTORY_FILE) != 0UL) && ((OpenOptions & FILE_OPEN_FOR_BACKUP_INTENT) != 0UL)) {
    ...
    if ((fullName.size() == 0) || (GetFileSize(ObjectAttributes->RootDirectory, nullptr) != INVALID_FILE_SIZE)) {
        return ::NtOpenFile(FileHandle, DesiredAccess, ObjectAttributes, IoStatusBlock, ShareAccess, OpenOptions);
    }

    try {
        std::pair<UnicodeString, bool> redir = applyReroute(READ_CONTEXT(), callContext, fullName);
        unique_ptr_deleter<OBJECT_ATTRIBUTES> adjustedAttributes = makeObjectAttributes(redir, ObjectAttributes);

        PRE_REALCALL
        res =
            ::NtOpenFile(FileHandle, DesiredAccess, adjustedAttributes.get(), IoStatusBlock, ShareAccess, OpenOptions);
        POST_REALCALL
        if (SUCCEEDED(res) && storePath) {
            // store the original search path for use during iteration
            READ_CONTEXT()->customData<SearchHandleMap>(SearchHandles)[*FileHandle] = static_cast<LPCWSTR>(fullName);
#pragma message("need to clean up this handle in CloseHandle call")
        ...
        PRE_REALCALL
        res = ::NtOpenFile(FileHandle, DesiredAccess, ObjectAttributes, IoStatusBlock, ShareAccess, OpenOptions);
        POST_REALCALL
    }
    HOOK_END
...
bool fileExists(const UnicodeString& filename) { return RtlDoesFileExists_U(static_cast<PCWSTR>(filename)) == TRUE; }

NTSTATUS WINAPI usvfs::hooks::NtCreateFile(PHANDLE FileHandle, ACCESS_MASK DesiredAccess,
                                           POBJECT_ATTRIBUTES ObjectAttributes, PIO_STATUS_BLOCK IoStatusBlock,
    ...
    HOOK_START_GROUP(MutExHookGroup::OPEN_FILE)
    if (!callContext.active()) {
        return ::NtCreateFile(FileHandle, DesiredAccess, ObjectAttributes, IoStatusBlock, AllocationSize,
    ...
    if (inPath.size() == 0) {
        spdlog::get("hooks")->info("failed to set from handle: {0}",
    ...
    { // limit context scope
        ...
        if (((CreateDisposition == FILE_SUPERSEDE) || (CreateDisposition == FILE_CREATE) ||
            ...
            std::pair<UnicodeString, UnicodeString> createTarget = findCreateTarget(context, inPath);
    ...
    PRE_REALCALL
    res = ::NtCreateFile(FileHandle, DesiredAccess, adjustedAttributes.get(), IoStatusBlock, AllocationSize,
                         FileAttributes, ShareAccess, CreateDisposition, CreateOptions, EaBuffer, EaLength);
    POST_REALCALL
...
NTSTATUS WINAPI usvfs::hooks::NtClose(HANDLE Handle) {
    ...
    if ((::GetFileType(Handle) == FILE_TYPE_DISK)) {
        ...
        {
            SearchHandleMap& searchHandles = context->customData<SearchHandleMap>(SearchHandles);
...
NTSTATUS WINAPI usvfs::hooks::NtQueryAttributesFile(POBJECT_ATTRIBUTES ObjectAttributes,
    ...
    std::pair<UnicodeString, bool> redir = applyReroute(READ_CONTEXT(), callContext, inPath);
    unique_ptr_deleter<OBJECT_ATTRIBUTES> adjustedAttributes = makeObjectAttributes(redir, ObjectAttributes);
    ...
    if (redir.second) {
        LOG_CALL().addParam("source", ObjectAttributes).addParam("rerouted", adjustedAttributes.get()).PARAMWRAP(res);
    }
...
NTSTATUS WINAPI usvfs::hooks::NtQueryFullAttributesFile(POBJECT_ATTRIBUTES ObjectAttributes,
