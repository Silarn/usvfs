...
class RerouteW {
  ...
  public:
    ...
    RerouteW(RerouteW&& reference)
        : m_Buffer(reference.m_Buffer), m_RealPath(reference.m_RealPath), m_Rerouted(reference.m_Rerouted),
    ...
    void insertMapping(const usvfs::HookContext::Ptr& context) {
        m_FileNode = context->redirectionTable().addFile(
            m_RealPath, usvfs::RedirectionDataLocal(string_cast<std::string>(m_FileName, CodePage::UTF8)));
    }
    ...
    static RerouteW create(const usvfs::HookContext::ConstPtr& context, const usvfs::HookCallContext& callContext,
                           const wchar_t* inPath, bool inverse = false) {
        RerouteW result;
        if ((inPath != nullptr) && (inPath[0] != L'\0') && !ush::startswith(inPath, L"hid#")) {
            ...
            if (callContext.active()) {
                ...
                if (result.m_FileNode.get() && !result.m_FileNode->data().linkTarget.empty()) {
                    result.m_Buffer =
                        string_cast<std::wstring>(result.m_FileNode->data().linkTarget.c_str(), CodePage::UTF8);
    ...
    static RerouteW createNew(const usvfs::HookContext::ConstPtr& context, const usvfs::HookCallContext& callContext,
                              LPCWSTR inPath) {
        ...
        if ((inPath != nullptr) && (inPath[0] != L'\0') && !ush::startswith(inPath, L"hid#")) {
            ...
            if (visitor.target.get() != nullptr) {
                ...
                result.m_Buffer = (fs::path(visitor.target->data().linkTarget.c_str()) / relativePath).wstring();
                try {
                    usvfs::FunctionGroupLock lock(usvfs::MutExHookGroup::ALL_GROUPS);
                    winapi::ex::wide::createPath(fs::path(result.m_Buffer).parent_path().wstring().c_str());
...
HMODULE WINAPI usvfs::hooks::LoadLibraryA(LPCSTR lpFileName) {
    return usvfs::hooks::LoadLibraryW(ush::string_cast<std::wstring>(lpFileName).c_str());
}
...
HMODULE WINAPI usvfs::hooks::LoadLibraryExA(LPCSTR lpFileName, HANDLE hFile, DWORD dwFlags) {
    return usvfs::hooks::LoadLibraryExW(ush::string_cast<std::wstring>(lpFileName).c_str(), hFile, dwFlags);
}
...
std::wstring getBinaryName(LPCWSTR applicationName, LPCWSTR lpCommandLine) {
    if (applicationName != nullptr) {
        ...
        while (space != nullptr) {
            ...
            if (winapi::ex::wide::fileExists(subString.c_str(), &isDirectory) && !isDirectory) {
                return subString;
            } else {
...
BOOL WINAPI usvfs::hooks::CreateProcessA(LPCSTR lpApplicationName, LPSTR lpCommandLine,
    ...
    { // scope for context lock
        ...
        applicationReroute = RerouteW::create(
            context, callContext,
            lpApplicationName != nullptr ? ush::string_cast<std::wstring>(lpApplicationName).c_str() : nullptr);
    ...
    res = ::CreateProcessA(appName, lpCommandLine != nullptr ? &cmdline[0] : nullptr, lpProcessAttributes,
    ...
    if (!blacklisted) {
        try {
            ...
            spdlog::get("hooks")->error("failed to inject into {0}: {1}", log::wrap(applicationReroute.fileName()),
                                        e.what());
        }
    }
    ...
    LOG_CALL().PARAM(applicationReroute.fileName()).PARAM(cmdline).PARAM(blacklisted).PARAM(res);
...
BOOL WINAPI usvfs::hooks::CreateProcessW(LPCWSTR lpApplicationName, LPWSTR lpCommandLine,
                                         LPSECURITY_ATTRIBUTES lpProcessAttributes,
                                         LPSECURITY_ATTRIBUTES lpThreadAttributes, BOOL bInheritHandles,
    ...
    PRE_REALCALL
    res = ::CreateProcessW(applicationReroute.fileName(), lpCommandLine != nullptr ? &cmdline[0] : nullptr,
...
HANDLE WINAPI usvfs::hooks::CreateFileA(LPCSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
                                        LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition,
                                        DWORD dwFlagsAndAttributes, HANDLE hTemplateFile) {
    return usvfs::hooks::CreateFileW(ush::string_cast<std::wstring>(lpFileName).c_str(), dwDesiredAccess, dwShareMode,
                                     lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile);
}

HANDLE WINAPI usvfs::hooks::CreateFileW(LPCWSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
                                        LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition,
                                        DWORD dwFlagsAndAttributes, HANDLE hTemplateFile) {
    ...
    RerouteW reroute;
    {
        ...
        if (((dwCreationDisposition == CREATE_ALWAYS) || (dwCreationDisposition == CREATE_NEW)) &&
            ...
            if (create) {
                ...
                winapi::ex::wide::createPath(target.parent_path().wstring().c_str(), lpSecurityAttributes);
            }
        }
    }

    PRE_REALCALL
    res = ::CreateFileW(reroute.fileName(), dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition,
                        dwFlagsAndAttributes, hTemplateFile);
    POST_REALCALL
...
BOOL WINAPI usvfs::hooks::GetFileAttributesExW(LPCWSTR lpFileName, GET_FILEEX_INFO_LEVELS fInfoLevelId,
    ...
    if (reroute.wasRerouted()) {
        LOG_CALL().PARAMWRAP(lpFileName).PARAMWRAP(reroute.fileName()).PARAMHEX(res).PARAMHEX(::GetLastError());
    }
...
DWORD WINAPI usvfs::hooks::GetFileAttributesW(LPCWSTR lpFileName) {
    ...
    if (reroute.wasRerouted()) {
        LOG_CALL().PARAMWRAP(lpFileName).PARAMWRAP(reroute.fileName()).PARAMHEX(res).PARAMHEX(callContext.lastError());
        ;
    }
...
DWORD WINAPI usvfs::hooks::SetFileAttributesW(LPCTSTR lpFileName, DWORD dwFileAttributes) {
    DWORD res = 0UL;
...
BOOL WINAPI usvfs::hooks::DeleteFileA(LPCSTR lpFileName) {
    return usvfs::hooks::DeleteFileW(ush::string_cast<std::wstring>(lpFileName).c_str());
}

BOOL WINAPI usvfs::hooks::MoveFileA(LPCSTR lpExistingFileName, LPCSTR lpNewFileName) {
    return usvfs::hooks::MoveFileW(ush::string_cast<std::wstring>(lpExistingFileName).c_str(),
                                   ush::string_cast<std::wstring>(lpNewFileName).c_str());
}

BOOL WINAPI usvfs::hooks::MoveFileW(LPCWSTR lpExistingFileName, LPCWSTR lpNewFileName) {
    BOOL res = FALSE;
...
BOOL WINAPI usvfs::hooks::MoveFileExA(LPCSTR lpExistingFileName, LPCSTR lpNewFileName, DWORD dwFlags) {
    return MoveFileExW(ush::string_cast<std::wstring>(lpExistingFileName).c_str(),
                       ush::string_cast<std::wstring>(lpNewFileName).c_str(), dwFlags);
}

BOOL WINAPI usvfs::hooks::MoveFileExW(LPCWSTR lpExistingFileName, LPCWSTR lpNewFileName, DWORD dwFlags) {
    BOOL res = FALSE;
...
BOOL WINAPI usvfs::hooks::CopyFileA(LPCSTR lpExistingFileName, LPCSTR lpNewFileName, BOOL bFailIfExists) {
    return CopyFileW(ush::string_cast<std::wstring>(lpExistingFileName).c_str(),
                     ush::string_cast<std::wstring>(lpNewFileName).c_str(), bFailIfExists);
}

BOOL WINAPI usvfs::hooks::CopyFileW(LPCWSTR lpExistingFileName, LPCWSTR lpNewFileName, BOOL bFailIfExists) {
    BOOL res = FALSE;
    ...
    PRE_REALCALL
    res = ::CopyFileW(readReroute.fileName(), writeReroute.fileName(), bFailIfExists);
    POST_REALCALL
...
BOOL WINAPI usvfs::hooks::CopyFileExA(LPCSTR lpExistingFileName, LPCSTR lpNewFileName,
                                      LPPROGRESS_ROUTINE lpProgressRoutine, LPVOID lpData, LPBOOL pbCancel,
                                      DWORD dwCopyFlags) {
    return ::CopyFileExW(ush::string_cast<std::wstring>(lpExistingFileName).c_str(),
                         ush::string_cast<std::wstring>(lpNewFileName).c_str(), lpProgressRoutine, lpData, pbCancel,
                         dwCopyFlags);
}

BOOL WINAPI usvfs::hooks::CopyFileExW(LPCWSTR lpExistingFileName, LPCWSTR lpNewFileName,
                                      LPPROGRESS_ROUTINE lpProgressRoutine, LPVOID lpData, LPBOOL pbCancel,
                                      DWORD dwCopyFlags) {
    BOOL res = FALSE;
    ...
    PRE_REALCALL
    res = ::CopyFileExW(readReroute.fileName(), writeReroute.fileName(), lpProgressRoutine, lpData, pbCancel,
                        dwCopyFlags);
    POST_REALCALL
...
DWORD WINAPI usvfs::hooks::GetCurrentDirectoryA(DWORD nBufferLength, LPSTR lpBuffer) {
    std::wstring buffer;
    buffer.resize(nBufferLength);
    DWORD res = usvfs::hooks::GetCurrentDirectoryW(nBufferLength, &buffer[0]);

    if (res > 0) {
        res = WideCharToMultiByte(CP_ACP, 0, buffer.c_str(), res, lpBuffer, nBufferLength, nullptr, nullptr);
    }

    return res;
}

DWORD WINAPI usvfs::hooks::GetCurrentDirectoryW(DWORD nBufferLength, LPWSTR lpBuffer) {
    DWORD res = FALSE;
    ...
    if (actualCWD.empty()) {
        ...
        ush::wcsncpy_sz(lpBuffer, &actualCWD[0], std::min(static_cast<size_t>(nBufferLength), actualCWD.size() + 1));
...
BOOL WINAPI usvfs::hooks::SetCurrentDirectoryA(LPCSTR lpPathName) {
    return usvfs::hooks::SetCurrentDirectoryW(ush::string_cast<std::wstring>(lpPathName).c_str());
}
...
DLLEXPORT BOOL WINAPI usvfs::hooks::CreateDirectoryW(LPCWSTR lpPathName, LPSECURITY_ATTRIBUTES lpSecurityAttributes) {
    BOOL res = FALSE;
    HOOK_START
...
DWORD WINAPI usvfs::hooks::GetFullPathNameW(LPCWSTR lpFileName, DWORD nBufferLength, LPWSTR lpBuffer,
                                            LPWSTR* lpFilePart) {
    DWORD res = 0UL;
    ...
    if (false) {
        LOG_CALL().PARAMWRAP(lpFileName).PARAMWRAP(lpBuffer).PARAM(res).PARAM(::GetLastError());
    }

    return res;
}

DWORD WINAPI usvfs::hooks::GetModuleFileNameW(HMODULE hModule, LPWSTR lpFilename, DWORD nSize) {
    DWORD res = 0UL;
    ...
    if ((res != 0) && callContext.active()) {
        RerouteW reroute = RerouteW::create(READ_CONTEXT(), callContext, lpFilename, true);
        if (reroute.wasRerouted()) {
...
DWORD WINAPI usvfs::hooks::GetModuleFileNameA(HMODULE hModule, LPSTR lpFilename, DWORD nSize) {
    std::unique_ptr<WCHAR[]> buffer(new WCHAR[nSize]);
    DWORD res = usvfs::hooks::GetModuleFileNameW(hModule, buffer.get(), nSize);
    if (res > 0) {
        WideCharToMultiByte(CP_ACP, 0, buffer.get(), -1, lpFilename, nSize, nullptr, nullptr);
        if (res > nSize) {
...
HMODULE WINAPI usvfs::hooks::GetModuleHandleW(LPCWSTR lpModuleName) {
    ...
    RerouteW reroute = RerouteW::create(READ_CONTEXT(), callContext, lpModuleName);
    PRE_REALCALL
...
BOOL WINAPI usvfs::hooks::GetModuleHandleExW(DWORD dwFlags, LPCWSTR lpModuleName, HMODULE* phModule) {
    BOOL res = FALSE;
...
BOOL WINAPI usvfs::hooks::GetModuleHandleExA(DWORD dwFlags, LPCSTR lpModuleName, HMODULE* phModule) {
    if (lpModuleName != nullptr) {
        return usvfs::hooks::GetModuleHandleExW(dwFlags, ush::string_cast<std::wstring>(lpModuleName).c_str(),
                                                phModule);
    } else {
...
BOOL WINAPI usvfs::hooks::GetFileVersionInfoExW(DWORD dwFlags, LPCWSTR lptstrFilename, DWORD dwHandle, DWORD dwLen,
                                                LPVOID lpData) {
    BOOL res = FALSE;
