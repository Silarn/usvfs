...
void WINAPI InitLogging(bool toConsole) { InitLoggingInternal(toConsole, false); }

extern "C" DLLEXPORT bool WINAPI GetLogMessages(char* buffer, size_t size, bool blocking) {
    buffer[0] = '\0';
    try {
        if (blocking) {
        ...
        _snprintf_s(buffer, size, _TRUNCATE, "Failed to retrieve log messages: %s", e.what());
        return false;
    }
}
    ...
    if (dbgDLL) {
        ...
        if (funcDump) {
...
#endif
            std::wstring parent = fs::path(dmpPath).parent_path().wstring();
            winapi::ex::wide::createPath(parent.c_str());
            HANDLE dumpFile =
                winapi::wide::createFile(dmpPath).createAlways().access(GENERIC_WRITE).share(FILE_SHARE_WRITE)();
            if (dumpFile != INVALID_HANDLE_VALUE) {
                ...
                if (success) {
                    if (logger != nullptr) {
                        logger->error("Crash dump created as \"{}\". Please send this file "
                                      "to the developer",
...
LONG WINAPI VEHandler(PEXCEPTION_POINTERS exceptionPtrs) {
    ...
    /*
    if (((exceptionPtrs->ExceptionRecord->ExceptionFlags &
    EXCEPTION_NONCONTINUABLE) != 0) ||
        (exceptionPtrs->ExceptionRecord->ExceptionCode == 0xe06d7363)) {
      // don't want to break on non-critical exceptions. 0xe06d7363 indicates a
    C++ exception. why are those marked non-continuable? return
    EXCEPTION_CONTINUE_SEARCH;
    }
    */

    if (RemoveVectoredExceptionHandler(exceptionHandler) == 0) {
        ::MessageBoxA(nullptr, "Failed to properly report windows exception, not daring to continue",
                      "Critical Error ^ 2", MB_OK);
        return EXCEPTION_CONTINUE_SEARCH;
    }
    ...
    try {
        ...
        uintptr_t exceptionAddress = reinterpret_cast<uintptr_t>(exceptionPtrs->ExceptionRecord->ExceptionAddress);
        ...
        if ((exceptionAddress < range.first) || (exceptionAddress > range.second)) {
            ...
            if (logger.get() != nullptr) {
                logger->warn("windows exception {0:x} from {1}", exceptionPtrs->ExceptionRecord->ExceptionCode,
            ...
            if (logger.get() != nullptr) {
                logger->critical("windows exception {0:x}", exceptionPtrs->ExceptionRecord->ExceptionCode);
            }
        }
    } catch (const std::exception& e) {
        if (logger.get() != nullptr) {
...
#pragma message("bug: if the ve handler is called, the process breaks")
    ...
    spdlog::get("usvfs")->debug("inithooks called {0} in process {1} (log level {2})", params->instanceName,
                                ::GetCurrentProcessId(), static_cast<int>(params->logLevel));
    spdlog::get("usvfs")->info("process name: {}", winapi::ansi::getModuleFileName(nullptr));
...
bool processStillActive(DWORD pid) {
    ...
    ON_BLOCK_EXIT([proc]() { ::CloseHandle(proc); });

    DWORD exitCode;
    if (!GetExitCodeProcess(proc, &exitCode)) {
        spdlog::get("usvfs")->warn("failed to query exit code on process {}: {}", pid, ::GetLastError());
        return false;
    } else {
...
bool assertPathExists(usvfs::RedirectionTreeContainer& table, LPCWSTR path) {
    ...
    for (auto iter = p.begin(); iter != p.end(); iter = ush::nextIter(iter, p.end())) {
        if (current->exists(iter->string().c_str())) {
            ...
            fs::path targetPath = current->data().linkTarget.size() > 0
                                      ? fs::path(current->data().linkTarget.c_str()) / *iter
                                      : *iter / "\\";
...
BOOL WINAPI VirtualLinkFile(LPCWSTR source, LPCWSTR destination, unsigned int flags) {
    ...
    try {
        ...
        if (extensions.find(fileExt) != extensions.end()) {
            ...
            context->inverseTable().addFile(fs::path(source), usvfs::RedirectionDataLocal(destinationU8), true);
        }
...
BOOL WINAPI VirtualLinkDirectoryStatic(LPCWSTR source, LPCWSTR destination, unsigned int flags) {
    ...
    try {
        ...
        context->redirectionTable().addDirectory(destination, usvfs::RedirectionDataLocal(sourceU8),
        ...
        if ((flags & LINKFLAG_RECURSIVE) != 0) {
            ...
            for (winapi::ex::wide::FileResult file : winapi::ex::wide::quickFindFiles(source, L"*")) {
                if (file.attributes & FILE_ATTRIBUTE_DIRECTORY) {
                    ...
                    std::string nameU8 = ush::string_cast<std::string>(file.fileName.c_str(), ush::CodePage::UTF8);
                    ...
                    if (extensions.find(fileExt) != extensions.end()) {
                        ...
                        context->inverseTable().addFile(fs::path(source) / nameU8,
...
BOOL WINAPI CreateProcessHooked(LPCWSTR lpApplicationName, LPWSTR lpCommandLine,
    ...
    fs::path p(applicationDirPath);
    try {
        usvfs::injectProcess(p.parent_path().wstring(), context->callParameters(), *lpProcessInformation);
    } catch (const std::exception& e) {
...
void WINAPI USVFSInitParameters(USVFSParameters* parameters, const char* instanceName, bool debugMode,
                                LogLevel logLevel) {
