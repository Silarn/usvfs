 ...
 *   - copy-on-write semantics (changes to files are done in a separate copy of
...
static const unsigned int LINKFLAG_MONITORCHANGES =
    0x00000002; // if set, changes to the source directory after the link
                ...
                // static link directory operations
static const unsigned int LINKFLAG_CREATETARGET =
    0x00000004; // if set, file creation (including move or copy) operations to
                // destination will be redirected to the source. Only one
                // createtarget can be set for a destination folder so this flag
...
/**
 * used internally to initialize a process at startup-time as a "slave". Don't
 * call directly
 */
